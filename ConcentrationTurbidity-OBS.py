...
y_Hach_err = 0.1*np.ones(len(y_Hach))    # FALTA AGREGAR LOS ERRORES.
...
y_OBS_err = 10*np.ones(len(x_OBS))
...
fit = Ajustar(lineal_con_offset,x_Hach,y_Hach,x_Hach_err_tot,y_Hach_err)
#%%
...
y_fit_OBS = lineal_con_offset(fit.beta, x_fit_OBS)


plt.figure()

plt.errorbar(x_Hach, y_Hach, xerr=x_Hach_err, yerr=None, fmt='o',color='darkred', label=r'Hach', ms=5.5, zorder=0)
plt.plot(x_fit_Hach, y_fit_Hach, color='red', label=r'Ajuste: $y = %.4f \; x + %.4f $'%(fit.beta[0],fit.beta[1]), lw=1, zorder=4)

plt.errorbar(x_OBS, y_OBS, xerr=x_OBS_err, yerr=None, fmt='o', color='darkblue', label=r'OBS', ms=5.5, zorder=0)
